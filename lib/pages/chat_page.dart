...
class ChatPageState extends State<ChatPage> {
  ...
  void readLocal() {
    ...
    groupChatIdGet(widget.arguments.peerId, currentUserId);
  ...
  Future<void> sendFromLocal() async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();

    _isOnline();
    i = prefs.getInt("count");
    int j=i!;
    if (_isConnectionSuccessful) {
      for (int k = 1; k <= i!; k++) {
        Map<String, dynamic> examResponseMap = {};
        final String? examResponseStr = prefs.getString('$k');
        if (examResponseStr != null) {
          examResponseMap = jsonDecode(examResponseStr) as Map<String, dynamic>;
        }
        final MessageChat exam = MessageChat.fromMap(examResponseMap);

        chatProvider.sendMessage(
          exam.content,
          exam.type,
          groupChatIdGet(exam.idTo, exam.idFrom),
          exam.idFrom,
          exam.idTo,
          _isConnectionSuccessful,
        );
        prefs.remove('messageChat$k');

        // j=j-1;
        // prefs.setInt("count", j);
  ...
  String groupChatIdGet(String peerId, String currentUser) {
    if (currentUser.compareTo(peerId) > 0) {
      groupChatId = '$currentUser-$peerId';
      return groupChatId;
    } else {
      groupChatId = '$peerId-$currentUser';
      return groupChatId;
    }
  }
}
